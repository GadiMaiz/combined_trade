...
class BitstampWSS(WSSAPI):
    ...
    def __init__(self, key=None, exclude=None, include_only=None, **kwargs):
		...
		self.channels = ['live_trades', 'live_trades_btceur',
                         ...
                         'live_orders_xrpbtc']
						 """
        self.channels = ['order_book', 'order_book_ltcusd', 'order_book_bchusd',
                         'live_orders', 'live_trades', 'live_trades_bchusd', 'live_trades_ltcusd']
        ...
        if exclude:
            if all(x in self.channels for x in exclude):
                ...
                raise ValueError("'exclude: must be a list of strings of"
                                 "valid channel name! %s" % self.channels)
        self.asks = {'BTC-USD' : [], 'BCH-USD' : [], 'LTC-USD' : [],}
        self.bids = {'BTC-USD' : [], 'BCH-USD' : [], 'LTC-USD' : [],}
    ...
    def ltcusd_lt_callback(self, data):
        self.live_trades_callback('LTCUSD', data)

    def bchusd_lt_callback(self, data):
        self.live_trades_callback('BCHUSD', data)
    ...
    def order_book_callback(self, pair, data):
        ...
        pair_crypto_type_dict = {'BTCUSD' : 'BTC-USD', 'BCHUSD' : 'BCH-USD', 'LTCUSD' : 'LTC-USD'}
    ...
    def bchusd_ob_callback(self, data):
        self.order_book_callback('BCHUSD', data)

    def ltcusd_ob_callback(self, data):
        self.order_book_callback('LTCUSD', data)
    ...
    def btcusd_lo_callback(self, data):
        self.live_orders_callback('BTC-USD', data, '')

    def ltcusd_lo_callback(self, data):
        self.live_orders_callback('LTC-USD', data, '')

    def btcusd_lo_callback_created(self, data):
        self.lo_callback_created("BTC-USD", data)

    def ltcusd_lo_callback_created(self, data):
        self.lo_callback_created("LTC-USD", data)

    def bchusd_lo_callback(self, data):
        self.live_orders_callback('BCH-USD', data, '')

    def bchusd_lo_callback_created(self, data):
        self.lo_callback_created("BCH-USD", data)

    def lo_callback_created(self, asset_pair, data):
        ex_type = ""
        ex_index = -1
        try:
            ...
            if order_dict['order_type'] == 1:
                ...
                index = 0
                ex_index = index
                while (index < len(self.asks[asset_pair]) and float(self.asks[asset_pair][index][0]) < price):
                    index += 1
                    ex_index = index
                new_ask = ["{0:.2f}".format(price), "{0:.8f}".format(order_dict['amount'])]
                ex_type = "adding ask"
                self.asks[asset_pair][index:index] = [new_ask]
                ex_type = "added ask"
                ...
                index = 0
                ex_index = index
                while (index < len(self.bids[asset_pair]) and float(self.bids[asset_pair][index][0]) > price):
                    index += 1
                    ex_index = index
                new_bid = ["{0:.2f}".format(price), "{0:.8f}".format(order_dict['amount'])]
                ex_type = "adding bid"
                self.bids[asset_pair][index:index] = [new_bid]
                ex_type = "added ask"
            ...
            print(self.bids)
        self.live_orders_callback(asset_pair + 'USD', data, 'created')
		
    def btcusd_lo_callback_deleted(self, data):
        self.lo_callback_deleted("BTC-USD", data)

    def ltcusd_lo_callback_deleted(self, data):
        self.lo_callback_deleted("LTC-USD", data)

    def bchusd_lo_callback_deleted(self, data):
        self.lo_callback_deleted("BCH-USD", data)

    def lo_callback_deleted(self, asset_pair, data):
        if len(self.asks[asset_pair]) > 0 or len (self.bids[asset_pair]) > 0:
            order_dict = json.loads(data)
            if order_dict['order_type'] == 1:
                ...
                while (index < len(self.asks[asset_pair]) and float(self.asks[asset_pair][index][0]) < price):
                    index += 1
                if index < len(self.asks[asset_pair]) and float(self.asks[asset_pair][index][0]) == float(price) and (float(self.asks[asset_pair][index][1]) == float(order_dict['amount']) or (float(order_dict['amount'] == 0))):
                    del self.asks[asset_pair][index]
                ...
                while (index < len(self.bids[asset_pair]) and float(self.bids[asset_pair][index][0]) > price):
                    index += 1
                if index < len(self.bids[asset_pair]) and float(self.bids[asset_pair][index][0]) == float(price) and (float(self.bids[asset_pair][index][1]) == float(order_dict['amount']) or (float(order_dict['amount'] == 0))):
                    del self.bids[asset_pair][index]
                #print ('bid deleted', self.bids)
            self.live_orders_callback(asset_pair + 'USD', data, 'deleted')

    def btcusd_lo_callback_changed(self, data):
        self.lo_callback_changed("BTC-USD", data)

    def ltcusd_lo_callback_changed(self, data):
        self.lo_callback_changed("LTC-USD", data)

    def bchusd_lo_callback_changed(self, data):
        self.lo_callback_changed("BCH-USD", data)

    def lo_callback_changed(self, asset_pair, data):
        if len(self.asks[asset_pair]) > 0 or len (self.bids[asset_pair]) > 0:
            order_dict = json.loads(data)
            if order_dict['order_type'] == 1:
                ...
                while (index < len(self.asks[asset_pair]) and float(self.asks[asset_pair][index][0]) < price):
                    index += 1
                if index < len(self.asks[asset_pair]) and float(self.asks[asset_pair][index][0]) == float(price):
                    changed_ask = ["{0:.2f}".format(price), "{0:.8f}".format(order_dict['amount'])]
                    self.asks[asset_pair][index] = changed_ask
                ...
                while (index < len(self.bids[asset_pair]) and float(self.bids[asset_pair][index][0]) > price):
                    index += 1
                if index < len(self.bids[asset_pair]) and float(self.bids[asset_pair][index][0]) == float(price):
                    changed_bid = ["{0:.2f}".format(price), "{0:.8f}".format(order_dict['amount'])]
                    self.bids[asset_pair][index] = changed_bid
                #print ('bid changed', self.bids)
            self.live_orders_callback(asset_pair + 'USD', data, 'changed')
    ...
    def _register_live_trades_channels(self):
        ...
        channels = {'live_trades': self.btcusd_lt_callback,
                    'live_trades_ltcusd': self.ltcusd_lt_callback,
                    'live_trades_bchusd': self.bchusd_lt_callback,
    ...
    def _register_order_book_channels(self):
        ...
        channels = {'order_book': self.btcusd_ob_callback,
                    'order_book_bchusd': self.bchusd_ob_callback,
                    'order_book_ltcusd': self.ltcusd_ob_callback,
    ...
    def _register_live_orders_channels(self):
        ...
        ltc_channels = {'live_orders_ltcusd': self.ltcusd_lo_callback}

        ltc_events = {'order_created': self.ltcusd_lo_callback_created,
                      'order_changed': self.ltcusd_lo_callback_changed,
                      'order_deleted': self.ltcusd_lo_callback_deleted}
        self._bind_channels_events(ltc_events, ltc_channels)
    ...
    def get_current_partial_book(self, asset_pair, book_size):
        partial_asks = self.asks[asset_pair][0:min(book_size, len(self.asks[asset_pair]) - 1)]
        partial_bids = self.bids[asset_pair][0:min(book_size, len(self.bids[asset_pair]) - 1)]
        result = {
            'asks': [],
            'bids': [],
        }
