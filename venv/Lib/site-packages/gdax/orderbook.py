...
class OrderBook(object):
    ...
    def get_current_partial_book(self, product_id, book_size):
        ...
        index = 0
        curr_ask_price, ask = self._asks[product_id].min_item()
        while (index < book_size and index < len(self._asks[product_id])):
            try:
                # There can be a race condition here, where a price point is
                # removed between these two ops
                this_ask = self._asks[product_id][curr_ask_price]
            except KeyError:
                continue
            this_sub_ask = {'price' : float(this_ask[0]['price']), 'size' : float(sum([order['size'] for order in this_ask]))}
            result['asks'].append(this_sub_ask)
            index += 1
            if index < len(self._asks[product_id]):
                curr_ask_price, ask = self._asks[product_id].succ_item(curr_ask_price)
            
        index = 0
        curr_bid_price, bid = self._bids[product_id].max_item()
        while (index < book_size and index < len(self._bids[product_id])):
            try:
                # There can be a race condition here, where a price point is
                # removed between these two ops
                this_bid = self._bids[product_id][curr_bid_price]
            except KeyError:
                continue
            this_sub_bid = {'price' : float(this_bid[0]['price']), 'size' : float(sum([order['size'] for order in this_bid]))}
            result['bids'].append(this_sub_bid)
            index += 1
            if index < len(self._bids[product_id]):
                curr_bid_price, bid = self._bids[product_id].prev_item(curr_bid_price)
    ...
    def get_all_asks(self, product_id):
        return self._asks[product_id]
    ...
    def get_all_bids(self, product_id):
        return self._bids[product_id]
